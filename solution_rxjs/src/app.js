...
class App {
  ...
  getTitlesForAddresses(res, addresses) {
    ...
    this.handleObservablesAndSubscriptions(res, promises, addresses)
  }
  ...
  handleObservablesAndSubscriptions(res, promises) {
    let remaining = promises.length

    const obsvPromises = promises.map(promise => {
      return Rx.Observable.fromPromise(promise)
    })

    obsvPromises.forEach(obsvPromise => {
      obsvPromise.subscribe(({ address, title }) => {
